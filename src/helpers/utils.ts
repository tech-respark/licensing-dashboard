...
export function timeDiffCalc(dateFuture: number, dateNow: number, status: any) {
 ...
 * @param {*} chars 
 */
export const TruncateText = (desc: string, chars: number) => {
...
let pluralize: any;
let underscore: any;
export function buildRelationshipData(type: any, ids: any[] | null) {
  let data: { type: any; id: any; }[] = []
...
export function cartIdentifier(storage: { get: (arg0: string) => null; set: (arg0: string, arg1: string) => void; }) {
...
export function parseJSON(response: { text: () => Promise<any>; status: any; ok: any; }) {
...
function formatFilterString(type: string, filter: { [x: string]: any; }) {
...
function formatQueryString(key: string, value: { [x: string]: any; }) {
...
function buildQueryParams({ includes, sort, limit, offset, filter }: any) {
  const query: any = {}
...
export function buildURL(endpoint: any, params: { includes: any; sort: any; limit: any; offset: any; filter: any; }) {
  if (
    params.includes ||
    params.sort ||
    params.limit ||
...
export function buildRequestBody(body: { options: any; data: any; }) {
  let parsedBody
  if (body) {
    if (body.options) {
      parsedBody = `{
...
export function getCredentials(storage: { get: (arg0: string) => string; }) {
  return JSON.parse(storage.get('moltinCredentials'))
}

export function tokenInvalid(config: { storage: any; client_id: any; }) {
...
export function hex2rgb(colour: string, alpha: any) {
  var r, g, b;
  if (colour.charAt(0) == "#") {
 ...
 */
export function deepMergeObjects(target: { [x: string]: any; }, source: { [x: string]: any; }) {
